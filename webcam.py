...
# Set the desired resolution and frame rateimport cv2
import mediapipe as mp
import numpy as np
import keyboard
import time
...
WIDTH = 500  # Width of the frame (you can adjust as needed)
HEIGHT = 500  # Height of the frame (you can adjust as needed)
        ...
        while cam.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    index_ring_distance = np.linalg.norm(index_finger_tip - ring_finger_tip)
                    index_pinky_distance = np.linalg.norm(index_finger_tip - pinky_tip)
                    middle_ring_distance = np.linalg.norm(middle_finger_tip - ring_finger_tip)
                    middle_pinky_distance = np.linalg.norm(middle_finger_tip - pinky_tip)
                    pinky_ring_distance = np.linalg.norm(pinky_tip - ring_finger_tip)

                    # Check if fingers are touching
                    if (thumb_index_distance <  GESTURE_THRESHOLD and
                        thumb_middle_distance > GESTURE_THRESHOLD and
                        thumb_ring_distance > GESTURE_THRESHOLD and
                        thumb_pinky_distance > GESTURE_THRESHOLD and
                        middle_ring_distance < GESTURE_THRESHOLD and
                        pinky_ring_distance < GESTURE_THRESHOLD):
                        print("Decrease Volume")
                        press_volume_down()  #pressing the volume down key
                           
                    elif (thumb_index_distance >  GESTURE_THRESHOLD and
                        thumb_middle_distance > GESTURE_THRESHOLD and
                        thumb_ring_distance > GESTURE_THRESHOLD and
                        thumb_pinky_distance > GESTURE_THRESHOLD and
                        middle_ring_distance < GESTURE_THRESHOLD and
                    ...
                    if (thumb_index_distance > GESTURE_THRESHOLD and
                        index_middle_distance > GESTURE_THRESHOLD and
                        middle_ring_distance > GESTURE_THRESHOLD and
                        pinky_ring_distance > GESTURE_THRESHOLD):
                        print("pause")
                        press_space()
                    ...
                    if (thumb_index_distance > GESTURE_THRESHOLD and
                        thumb_middle_distance > GESTURE_THRESHOLD and
                        index_middle_distance < GESTURE_THRESHOLD and
                        index_ring_distance > GESTURE_THRESHOLD and
                        index_pinky_distance > GESTURE_THRESHOLD and
                        middle_ring_distance > GESTURE_THRESHOLD and
                        middle_pinky_distance > GESTURE_THRESHOLD and
                        thumb_ring_distance < GESTURE_THRESHOLD and
                        thumb_pinky_distance < GESTURE_THRESHOLD and
                        pinky_ring_distance < GESTURE_THRESHOLD):
                        print("switch")
